  ...
  return (
    <main className="flex-1 overflow-y-auto bg-bg-deepest">
      <div className="p-6 lg:p-8 2xl:p-10">
        ...
        {/* Three-panel layout */}
        <div className="grid gap-8 2xl:gap-10 lg:grid-cols-[280px_minmax(0,1fr)_300px] 2xl:grid-cols-[300px_minmax(0,1fr)_320px]">
          {/* ===== LEFT PANEL — Threat Categories ===== */}
          <div className="space-y-6">
            <h2 className="text-xs font-bold text-primary uppercase tracking-[0.1em] mb-4 2xl:mb-5">
              Threat Categories
            </h2>
            ...
            {(['critical', 'high', 'medium'] as const).map((sev, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-card rounded-[18px] border border-border p-4 card-glow flex items-center gap-4"
                >
                  <div
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-text-muted capitalize">{sev} Alerts</p>
                    <p className="text-xl font-bold text-text-primary font-display">{count}</p>
                  </div>
                  <div
              ...
              <div className="h-[260px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={eventTypes} barSize={20}>
                    ...
                    />
                    <Bar dataKey="count" fill="var(--color-primary-active)" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="space-y-2">
                {topIPs.map((ip, idx) => (
                  <div
                    key={ip.source_ip}
                    className="flex items-center justify-between bg-bg-elevated rounded-lg px-4 py-2.5 border border-border/50"
                  >
                ...
                {recentAlerts.slice(0, 5).map((alert) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span
                        className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full badge-${alert.severity}`}
                      >
                        {alert.severity}
                      </span>
                    </div>
          ...
          <div className="space-y-6">
            ...
            <motion.div
              ...
              className="bg-card rounded-[18px] border border-border p-4 card-glow space-y-4"
            >
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  <span className="text-xs text-text-muted">Training Samples</span>
                  <span className="text-xs font-semibold text-text-primary">
                    {mlStatus?.trainingSamples ?? 0}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs text-text-muted">Training Runs</span>
                  <span className="text-xs font-semibold text-text-primary">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.3, duration: 0.25, ease: 'easeOut' }}
              className="bg-card rounded-[18px] border border-border p-4 card-glow space-y-3"
            >
              <div className="flex items-center gap-2 mb-1">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.4, duration: 0.25, ease: 'easeOut' }}
              className="bg-card rounded-[18px] border border-border p-4 card-glow"
            >
...
const StatusRow = ({ label, status }: { label: string; status: string }) => (
  <div className="flex items-center justify-between">
    ...
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-[10px] font-medium text-success">{status}</span>
    </div>
  </div>
);